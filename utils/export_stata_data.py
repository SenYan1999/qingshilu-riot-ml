...
def export_stata(create_weather_grain=False, grain_idx=0):
    ...
    with open('data/four_infer_entries.json', 'r') as f:
        tri_data = json.load(f)
    for i in range(len(tri_data)):
        year, month = to_solar(tri_data[i]['year'], tri_data[i]['month'])
        tri_data[i]['year'] = year
        tri_data[i]['month'] = month
    ...
    df.to_stata(f'data/stata/export/quadruple_{suffix}_stata_validation_weather_grain_month.dta', write_index=False)
    ...
    year_df.to_stata(f'data/stata/export/quadruple_{suffix}_stata_validation_weather_grain_year.dta', write_index=False)

def export_stata_mix_prob(create_weather_grain=False, grain_idx=0):
    raw_pref2code = pd.read_excel('data/stata/pref2code.xlsx')
    pref2code = {name: code for name, code in zip(raw_pref2code['中俄尼布楚条约待议地区'].tolist(), raw_pref2code['pref_code'].tolist())}
    for pref, code in pref2code.items():
        if str(code) == '.' or str(code).strip() == '':
    ...
    years, months, riot_types, prefectures, peasant_prob, jieshe_prob, wuzhuang_prob = [], [], [], [], [], [], []
    qing_range = list(range(1644, 1912))
    ...
    for entry in tri_data:
        ...
        for prefecture in entry['prefectures']:
            ...
            riot_types.append(entry['RiotType'])
            prefectures.append(prefecture)
            peasant_prob.append(entry['RiotProb'][0])
            jieshe_prob.append(entry['RiotProb'][1])
            wuzhuang_prob.append(entry['RiotProb'][2])
    ...
    for year in qing_range:
        for month in range(1, 13):
            final_data['%d-%d' % (year, month)] = {pref: {'riot_count': 0, 'jieshe_prob_sum': 0, 'wuzhuang_prob_sum': 0, 'peasant_prob_sum': 0, 'jieshe_count': 0, 'peasants_count': 0, 'wuzhuang_count': 0, 'month_counts': month_count['%d-%d' % (year, month)]} for pref in pref2code.values()}

    for year, month, rt, pref, pp, jp, wp in zip(years, months, riot_types, prefectures, peasant_prob, jieshe_prob, wuzhuang_prob):
        if rt == 'Nan':
            continue
        pref = pref2code[pref]
        final_data['%d-%d' % (year, month)][pref]['riot_count'] = final_data['%d-%d' % (year, month)][pref]['riot_count'] + 1
        final_data['%d-%d' % (year, month)][pref]['jieshe_prob_sum'] = final_data['%d-%d' % (year, month)][pref]['jieshe_prob_sum'] + jp
        final_data['%d-%d' % (year, month)][pref]['peasant_prob_sum'] = final_data['%d-%d' % (year, month)][pref]['peasant_prob_sum'] + pp
        final_data['%d-%d' % (year, month)][pref]['wuzhuang_prob_sum'] = final_data['%d-%d' % (year, month)][pref]['wuzhuang_prob_sum'] + wp
        if rt == 'Peasant':
            final_data['%d-%d' % (year, month)][pref]['peasants_count'] = final_data['%d-%d' % (year, month)][pref]['peasants_count'] + 1
        elif rt == 'Secret Party':
            final_data['%d-%d' % (year, month)][pref]['jieshe_count'] = final_data['%d-%d' % (year, month)][pref]['jieshe_count'] + 1
    ...
    all_years, all_months, all_prefs, all_rc, all_jc, all_pc, all_wc, all_mc, all_pp, all_jp, all_wp = [], [], [], [], [], [], [], [], [], [], []
    for key, items in final_data.items():
        year, month = key.split('-')
        for pref, info in items.items():
            all_years.append(year)
            ...
            all_wc.append(info['wuzhuang_count'])
            all_mc.append(info['month_counts'])
            all_pp.append(info['peasant_prob_sum'])
            all_jp.append(info['jieshe_prob_sum'])
            all_wp.append(info['wuzhuang_prob_sum'])
    ...
    df = pd.DataFrame({
        ...
        'month_counts': all_mc,
        'riot_count': all_rc,
        'jieshe_prob_sum': all_jp,
        'peasants_prob_sum': all_pp,
        'wuzhuang_prob_sum': all_wp,
        'jieshe_count': all_jc,
        'peasants_count': all_pc,
        'wuzhuang_count': all_wc,
        'weather_last_year': all_weather_last_year,
    ...
    df_copy['peasants_prob'] = np.where(df_copy['riot_count'] == 0, 0, df_copy['peasants_prob_sum'] / df_copy['riot_count'])
    df_copy['jieshe_prob'] = np.where(df_copy['riot_count'] == 0, 0, df_copy['jieshe_prob_sum'] / df_copy['riot_count'])
    df_copy['wuzhuang_prob'] = np.where(df_copy['riot_count'] == 0, 0, df_copy['wuzhuang_prob_sum'] / df_copy['riot_count'])
    df_copy = df_copy.drop(columns=['peasants_prob_sum', 'jieshe_prob_sum', 'wuzhuang_prob_sum'])
    df_copy.to_stata(f'data/stata/export/mix_prob_{suffix}_stata_validation_weather_grain_month.dta', write_index=False)

    year_df = df.groupby(['year', 'prefecture']).agg({'riot_count':'sum', 'jieshe_count': 'sum',
                                                'peasants_count': 'sum',
                                                'wuzhuang_count': 'sum',
                                                'peasants_prob_sum': 'sum',
                                                'wuzhuang_prob_sum': 'sum',
                                                'jieshe_prob_sum': 'sum',
                                                'weather_last_year': 'first', 'weather':'first',
                                                'grain_min_last_year': 'first', 'grain_max_last_year': 'first',
                                                'grain_min_current_year': 'first', 'grain_max_current_year': 'first',
                                                ...
                                                'grain_last_fall': 'first', 'grain_current_fall': 'first',
                                                'grain_last_winter': 'first', 'grain_current_winter': 'first'}).reset_index()
    year_df['peasants_prob'] = np.where(year_df['riot_count'] == 0, 0, year_df['peasants_prob_sum'] / year_df['riot_count'])
    year_df['jieshe_prob'] = np.where(year_df['riot_count'] == 0, 0, year_df['jieshe_prob_sum'] / year_df['riot_count'])
    year_df['wuzhuang_prob'] = np.where(year_df['riot_count'] == 0, 0, year_df['wuzhuang_prob_sum'] / year_df['riot_count'])
    year_df = year_df.drop(columns=['peasants_prob_sum', 'jieshe_prob_sum', 'wuzhuang_prob_sum'])
    year_df.to_stata(f'data/stata/export/mix_prob_{suffix}_stata_validation_weather_grain_year.dta', write_index=False)
...
if __name__ == '__main__':
    ...
    export_stata_mix_prob(create_weather_grain=False, grain_idx=17)
