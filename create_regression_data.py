...
def export_stata(create_weather_grain=False, grain_idx=0):
    raw_pref2code = pd.read_excel('data/pref2code.xlsx')
    ...
    if create_weather_grain:
        raw_grain = pd.read_stata('data/China_pre_post.dta')
        ...
        with open(f'data/pref_year_month_{grain_idx}.json', 'wb') as f:
            pickle.dump(pref_year_month_grain, f)
    else:
        with open(f'data/pref_year_month_{grain_idx}.json', 'rb') as f:
    ...
    if create_weather_grain:
        raw_weather = pd.read_stata('data/pref_weather.dta')
        ...
        with open('data/pref_year_weather.json', 'wb') as f:
            pickle.dump(pref_year_weather, f)
    else:
        with open('data/pref_year_weather.json', 'rb') as f:
    ...
    for year, month, pref in tqdm(zip(all_years, all_months, all_prefs), desc='Parsing Grain Price...', total=len(all_years)):
        try:
    ...
    df.to_stata(f'data/{suffix}_stata_validation_weather_grain_month.dta', write_index=False)
    ...
    year_df.to_stata(f'data/{suffix}_stata_validation_weather_grain_year.dta', write_index=False)
